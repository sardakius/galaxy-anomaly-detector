...
DATA_DIR = {
    ...
    "Mean Absolute Error" : {'roc_auc': [], 'pr_auc': [], 'f1_score': []},
    "χ2 Error" : {'roc_auc': [], 'pr_auc': [], 'f1_score': []}
}
...
# plotting
for (loss, sub_dict) in DATA_DIR.items():
    plt.scatter(sub_dict['roc_auc'], sub_dict['pr_auc'], s=np.array(sub_dict['f1_score'])*20, label=loss)
...
plt.title("AUC Scatterplot (Loss View)")
